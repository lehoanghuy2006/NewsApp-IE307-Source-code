...
import axios from 'axios'
 export default Trend = ({navigation}) =>{
    const [news, setNews] = useState([]);
  
  useEffect(() => {
    const fetchNews = async () => {
      const response = await axios.get(
        'https://newsdata.io/api/1/news?country=vi&category=top&apikey=pub_35039f80da0243cb8216921576aefeb1305ce'
      );
    ...
    <SafeAreaView style={styles.homeContainer}>
        <View style={styles.headerBox}>
           ...
           <Text style={styles.pageTitle}>Trend</Text>
           <View></View>
        </View>
