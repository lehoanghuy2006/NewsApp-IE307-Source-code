...
import axios from 'axios'
 export default Tech = ({navigation}) =>{
    const [news, setNews] = useState([]);

  useEffect(() => {
    const fetchNews = async () => {
      const response = await axios.get(
        'https://newsdata.io/api/1/news?country=vi&category=technology&apikey=pub_35039f80da0243cb8216921576aefeb1305ce'
      );
    ...
    <SafeAreaView style={styles.homeContainer}>
        <View style={styles.headerBox}>
          <View style={styles.backBtn} >
           ...
           <Text style={styles.pageTitle}>Công Nghệ</Text>
           <View></View>
        </View>
